  ...
  static async makeShortLink(req: Request, res: Response) {
    ...
    if (!url)
      return res
        ...
        .json(ApiResponse.error(Messages.URL_REQUIRED));
    ...
    if (!Valid.data)
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json(ApiResponse.error(Messages.INVALID_URL));
    ...
    if (has_link) {
      ...
      return res
        ...
        .json(
          ApiResponse.success(
            Messages.URL_EXISTS,
            `${process.env.BASE_URL}/${has_link.short}`
          )
        );
    }
    ...
    try {
      ...
      return res
        ...
        .json(ApiResponse.error(Messages.INTERNAL_SERVER_ERROR));
    }
  }
    ...
    try {
      ...
      if (!urlOriginal)
        return res
          ...
          .json(ApiResponse.error(Messages.URL_NOT_FIND));
